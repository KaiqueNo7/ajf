<div>
    ...
    <form wire:submit='create()'>
        <div class="flex items-center bg-gray-700 py-2 rounded-lg p-4">
            <input type="hidden" wire:model='property_id' value="{{ $property_id }}">
            <input type="text" wire:model='text' class="focus:outline-none focus:ring-0 focus:border-transparent appearance-none bg-transparent border-none w-full text-white mr-3 py-1 px-2 leading-tight" placeholder="Informações adicionais" required>
    ...
    @error('additionalInformation') <span class="error text-red-600">{{ $message }}</span> @enderror
</div>
